...
def getData(fileName): 
    ...
    # The header is discarded simply by reading it now and not reading it in iteration
    # This way I dont add it to the list and I dont need to remove it later
    for line in dataFile:
        d, m = line.split() 
    # We need to be sure the document is in correct format - it has only two columns with desired values
    # d is first value, m is second value
...
def testErrors(ntrials=10000,npts=100):
    results = [0] * ntrials 
    # creates a list ntrials long with 0 value at each index
    # otherwise, I couldnt replace the value and would have to append it to empty list
...
def fitData2(fileName):
    ...
    # testing predictive power on new cases
    xVals = pylab.array(xVals)
    yVals = pylab.array(yVals)
    xVals = xVals*9.81  # convert mass to force (F = mg)
...
def rSquare(measured, estimated): # Coefficient of determination
    """measured: one dimensional array of measured values
       estimate: one dimensional array of predicted values"""
    SEE = ((estimated - measured)**2).sum() # sum of estimated error
    mMean = measured.sum()/float(len(measured))
    MV = ((mMean - measured)**2).sum()
    return 1 - SEE/MV # variance of the error over variance of the data
    # The fit is the better the closer to 1 the number is
