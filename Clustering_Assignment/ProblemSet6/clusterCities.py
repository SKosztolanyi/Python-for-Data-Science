...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        SLDist = self.points[0].distance(other.points[0])
        for p0 in self.points:
            for p1 in other.points:
                if p0.distance(p1) < SLDist:
                    SLDist = p0.distance(p1)
        return SLDist
    ...
    def maxLinkageDist(self, other):
        ...
        self and the other point is from other. Uses the 
        Euclidean dist between 2 points, defined in Point."""
        MaxLDist = self.points[0].distance(other.points[0])
        for p0 in self.points:
            for p1 in other.points:
                if p0.distance(p1) > MaxLDist:
                    MaxLDist = p0.distance(p1)
        return MaxLDist
    ...
    def averageLinkageDist(self, other):
        ...
        for p0 in self.points:
            for p1 in other.points:
                Distances += abs(p0.distance(p1))
        return Distances/(len(self.points)*len(other.points))
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        for point in c2.members():
            points.append(point)
        newC = Cluster(points, type(point))
